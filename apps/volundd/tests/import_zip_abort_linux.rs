...
#[tokio::test]
async fn aborted_zip_request_cleans_extraction_before_retry() {
    ...
    let parts: Vec<_> = std::fs::read_dir(&directory)
        .unwrap()
        .map(Result::unwrap)
    ...
    assert_eq!(before["uploadedBytes"], zip.len());
    let retry = f
        .router
        .clone()
        .oneshot(upload_request(id, item, zip.clone()))
    ...
    let cleanup = volundd::import_cleanup::run(&f.db, &f.root.join("incoming"))
        .await
        .unwrap();
    assert_eq!(cleanup.cleaned_drafts, 0);
    let actual: u64 = std::fs::read_dir(&directory)
        .unwrap()
        .map(|e| e.unwrap().metadata().unwrap().len())
        .sum();
    let (_, after) = support::get_json(&f.router, "/api/v1/imports/storage").await;
    assert_eq!(actual, u64::try_from(zip.len() + 4).unwrap());
    assert_eq!(after["uploadedBytes"], zip.len() + 4);
    assert_eq!(after["reservedBytes"], zip.len() + 4);
}
